...
from .DTGN_components import ComponentList, LMC_conv_block, GatedOh, Sub_DTGN
from simple_parsing import choice
device = nn_device = 'cuda' if torch.cuda.is_available() else 'cpu'

class DTGN_net(ModularBaseNet):
    @dataclass
    class Options(ModularBaseNet.Options):  
        ...
        #deviation threshold
        deviation_threshold: float = 2.5   ####3.
        #test time oracle
    ...
    def __init__(self, options:Options = Options(), module_options:LMC_conv_block.Options=LMC_conv_block.Options(), i_size: int = 28, channels:int = 1, hidden_size=64, num_classes:int=5):
        super(DTGN_net, self).__init__(options, i_size, channels, hidden_size, num_classes)

        self.n_DTGN_modules = torch.tensor([2.,2.,2.,2.])
        self.MODEL_DEPTH = 2

        self.args: DTGN_net.Options = copy.copy(options)
        ...
        self.components: List[List[Sub_DTGN]] = self.components

        self.init_modules()    

        # self.modules_to_unfreeze_func:List[LMC_conv_block]=[]
        self.modules_to_unfreeze_func:List[Sub_DTGN]=[]
    ...
    def init_modules(self):
        ...
        for i in range(self.depth):
            ...
            sub_layer_num_modules = self.n_modules[i]*2
            print("\nBuilding %d Total Leaf" % sub_layer_num_modules.item())
            ...
            for m_i in range(self.n_modules_at_layer(i)):
                ...
                layer_n_modules = sub_layer_num_modules
                self.block_constructor=Sub_DTGN
                conv = self.block_constructor(out_h, channels_in, hidden_size, out_h, name=f'components.{i}.{m_i}', module_type=module_type, initial_inv_block_lr=self.lr_structural,
                                                            deviation_threshold=self.deviation_threshold, freeze_module_after_step_limit=self.args.freeze_module_after_step_limit, deeper=deeper, layer_n_modules = layer_n_modules,
    ...
    def add_modules(self, block_constructor=None, at_layer=None, strict=False, module_i=None, verbose=True, init_stats=False):
        ...
        for l, layer in enumerate(self.components):  
            ...
            if l != at_layer and at_layer!=None:
                ...
                sub_DTGN = layer[0]
                in_h = sub_DTGN.in_h
                channels_in = sub_DTGN.in_channels
                channels = sub_DTGN.out_channels
                i_size = sub_DTGN.i_size
                deeper = sub_DTGN.deeper
                module_type = sub_DTGN.module_type
                self.module_options.kernel_size=sub_DTGN.args.kernel_size
                self.module_options.dropout=sub_DTGN.args.dropout
                # out_h = layer[0].out_h       
                ################################################
                init_params = None         
                if self.args.module_init == 'identical':         
    ...
    def get_optimizers(self):
        ...
        structure_names = ['inv_block', 'structure','SubDTGN_inv']
        model_params_test = []
        model_params_name = []
        for n, param in self.named_parameters():
            if not any(map(n.__contains__, structure_names)) and param.requires_grad:
                model_params_test.append(param)
        ...
        for layer in self.components:
            for component in layer:
                if hasattr(component, 'SubDTGN_inv'):
                    params = list(filter(lambda p: p.requires_grad, component.SubDTGN_inv.parameters()))
                    if len(params)>0:
                        structure_param_groups.append({'params': params, 'lr': component.block_lr_buffer, 'name': component.name})

                    for n_SubDTGN_inv, param_SubDTGN_inv in component.SubDTGN_inv.named_parameters():
                        if param_SubDTGN_inv.requires_grad:
                            params_test.append(param_SubDTGN_inv)
                            params_name_test.append(n_SubDTGN_inv)

                    for sub_DTGN in component.SubDTGN_conv:
                        if hasattr(sub_DTGN, 'inv_block'):
                            params_lmc = list(filter(lambda p: p.requires_grad, sub_DTGN.inv_block.parameters()))
                            if len(params_lmc)>0:
                                structure_param_groups.append({'params': params_lmc, 'lr': sub_DTGN.block_lr_buffer, 'name': sub_DTGN.name})

                        for n_sub_DTGN, param_sub_DTGN in sub_DTGN.inv_block.named_parameters():
                            if param_sub_DTGN.requires_grad :
                                params_test.append(param_sub_DTGN)
                                params_name_test.append(n_sub_DTGN)
    ...
    def forward_layer(self, layer, X, beam_results, temp, task_id,  inner_loop, decoder, record_running_stats, env_assignments, detach_head,projection_phase_flag, DTGN_dropout_raito,str_prior_temp, info='', log_at_this_iter_flag=False):
        ...
        if len(beam_results)>0:   
            ...
            for X in beam_results:
                ...
                beam_str_loss_SubDTGN = []
                beam_fixed_functional_layer = []
                for _, conv in enumerate(layer):
                     DTGN_dropout = DTGN_Dropout(DTGN_dropout_raito)
                     DTGN_dropout.forward()
                     a = DTGN_dropout.mask
                     b = (a == 1)
                     if b:
                            #collect outputs from each module at layer for each beam path
                            conv.args.str_prior_factor = self.args.str_prior_factor
                            conv.args.K_train = self.args.K_train
                            conv.args.K_test = self.args.K_test
                            conv.args.regime= self.args.regime
                            conv.args.mask_str_loss = self.args.mask_str_loss
                            conv.args.concat= self.args.concat
                            conv.args.multihead = self.args.multihead
                            conv.args.module_oracle = self.args.module_oracle
                            conv.args.optmize_structure_only_free_modules=self.args.optmize_structure_only_free_modules
                            func_out, str_to_minimize, str_from_backup, deviation, outliear_score, outlier, is_learned, fixed_functional, str_loss_SubDTGN = conv(X, info=info, log_at_this_iter= log_at_this_iter_flag,task_id=task_id, temp=temp, inner_loop=inner_loop,added =False, decoder = decoder, record_running_stats=record_running_stats, env_assignments=env_assignments, detach_head=detach_head, str_prior_temp = str_prior_temp,projection_phase_flag =projection_phase_flag,beam_results =beam_results)
                            ...
                            beam_str_loss_SubDTGN.append(str_loss_SubDTGN)
                ...
                str_loss_SubDTGN_layer.append(beam_str_loss_SubDTGN)
            ...
            for _, conv in enumerate(layer):
                dropout = torch.tensor([0])

                if bernoulli_list.__len__() > 0:
                    ii = 0
                    for i, n in enumerate(bernoulli_list):
                        ii += n
                    if ii == 0:
                        dropout = torch.tensor([1])
                    else:
                         DTGN_dropout = DTGN_Dropout(DTGN_dropout_raito)
                         DTGN_dropout.forward()
                         dropout = DTGN_dropout.mask
                else:
                    DTGN_dropout = DTGN_Dropout(DTGN_dropout_raito)
                    DTGN_dropout.forward()
                    dropout = DTGN_dropout.mask

                bernoulli_list.append(dropout)
                b = (dropout == 1)
                if b:
                    ...
                    conv.args.module_oracle = self.args.module_oracle
                    conv.args.optmize_structure_only_free_modules=self.args.optmize_structure_only_free_modules
                    func_out, str_to_minimize, str_from_backup, deviation, outliear_score, outlier, is_learned, fixed_functional, str_loss_SubDTGN = conv(X, info=info, log_at_this_iter= log_at_this_iter_flag,task_id=task_id, temp=temp, inner_loop=inner_loop, decoder = decoder, record_running_stats=record_running_stats, env_assignments=env_assignments, detach_head=detach_head, str_prior_temp = str_prior_temp,projection_phase_flag =projection_phase_flag,beam_results =beam_results)
                    ...
                    str_loss_SubDTGN_layer.append(str_loss_SubDTGN)
            ...
            str_loss_layer_SubDTGN_layer = torch.stack(str_loss_SubDTGN_layer)
            ...
            return X, str_layer, str_layer_from_backup_layer, str_layer_for_masks_layer, deviation_layer, z_scores_layer, fixed_functional_layer, module_learned_layer, str_loss_layer_SubDTGN_layer

    def forward(self, X, task_id=None, temp=None, inner_loop=False, decoder = None, record_running_stats=False, env_assignments=None, info='', detach_head=False, train_flag = True, str_prior_temp=None, **kwargs):
        #start = time.time()
        if train_flag == True:
            DTGN_dropout_raito = torch.tensor([0.85])         #This parameter needs to be adjusted
        else:
             DTGN_dropout_raito = torch.tensor([1.0])
        ...
        str_loss_SubDTGN_total = torch.zeros(self.depth).to(device)
        ...
        ###############################        
        for l, layer in enumerate(self.components):                     
            fw_layer = self.forward_layer(layer, X, beam_results ,temp = temp, info=info, log_at_this_iter_flag=(self.log_at_this_iter_flag and not inner_loop), task_id=task_id, inner_loop=inner_loop, decoder = decoder, record_running_stats=record_running_stats, env_assignments=env_assignments, detach_head=detach_head, str_prior_temp=str_prior_temp,projection_phase_flag =projection_phase_flag, DTGN_dropout_raito=DTGN_dropout_raito)
            X_tmp, str_loss, str_loss_from_backup, str_loss_detached_for_mask, deviation_layer, z_scores_layer, fixed_modules_function_layer, fixed_module, str_loss_SubDTGN= fw_layer
                    ...
                    if added or (self.args.treat_unfreezing_as_addition and params_changed):
                        if not self.args.no_projection_phase:
                            ...
                            for ni in new_m_idx:                  
                                ...
                                for DTGN_ni in layer[ni].SubDTGN_conv:
                                     DTGN_ni.args.detach_structural=True
                            #layers below
            ...
            if not inner_loop:      
                if torch.sum(fixed_modules_function_layer) != len(fixed_modules_function_layer) or self.args.regime=='normal':      
                    ...
                    str_loss_SubDTGN_total[l] += str_loss_SubDTGN.mean()
            else:
        ...
        str_loss_SubDTGN_total = torch.sum(str_loss_SubDTGN_total)
        ...
        factor_SubDTGN_loss = 0   #0.20
        str_loss_total += factor_SubDTGN_loss*str_loss_SubDTGN_total.mean()
        ...
        info={'deviation_mask':deviation_mask , 'outlier_signal': outlier_signal, 'fixed_modules': fixed_modules, 'selected_decoder': decoder_idx}), str_loss_SubDTGN_total#, 'modules_lr': modules_lr}) 'add_modules_layers': add_modules_at_leayer,
    ...
    def handle_outer_loop_finished(self, finished_outer_loop: bool):
        for l in self.components:
            for c in l:
                ...
                for x in c.SubDTGN_conv:
                    x.on_finished_outer_loop(finished=finished_outer_loop)
...
class DTGN_Dropout:
    ...
    def forward(self):
        temp_dropout = torch.distributions.bernoulli.Bernoulli(self.dropout_ratio)
        f_dropout = temp_dropout.sample()
        self.mask = f_dropout
