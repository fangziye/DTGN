...
from main_transfer_fzy import (ArgsGenerator, create_dataloader_ctrl,
...
class ArgsGenerator(ArgsGenerator):
    ...
    wdecay: float = 0 #weight decay [0,
    lr: float = 0.01 # learning rate
    copy_batchstats: int = 1 #if 'True'
    keep_bn_in_eval_after_freeze: bool
    # FZY
    momentum_bn: float = 0.1
    pr_name: str = 'lmc_cr'
    track_running_stats_bn: int = 1
    task_sequence: str = 's_ood'
    shuffle_test: int = 0
    epochs: int = 50
    regenerate_seed: int = 0
    hidden_size: int = 64
    keep_bn_in_eval_after_freeze=0
    module_init: str = 'none'
    multihead: str = 'usual'
    normalize_data: int = 1
    use_structural: int = 0
    #
    #if 'True' uses entropy to select path with the lowest walue in case of
