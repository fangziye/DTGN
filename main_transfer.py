...
from Methods.models.DTGN import DTGN_net
...
class ArgsGenerator():       
    ##############################
    #learning process relate
    projection_phase_length: int = 15   #This parameter needs to be adjusted
    fix_layers_below_on_addition: int = 0    #This parameter needs to be adjusted
    deviation_threshold: float = 4.5  #This parameter needs to be adjusted
    reg_factor: float = 2   #This parameter needs to be adjusted
    temp: float = 1    #This parameter needs to be adjusted
    anneal: int = 0      #This parameter needs to be adjusted
    lr_structural: float = 0.001 #-     #This parameter needs to be adjusted
    regime: str = choice('multitask','cl', default='cl')
    module_init: str = choice('none','mean','previously_active', 'most_likely', default='mean')      #This parameter needs to be adjusted
    mask_str_loss: int = 1       #This parameter can be adjusted if you need it
    structure_inv: str = choice('linear_no_act', 'pool_only_large_lin_no_act', 'linear_act', 'ae', default='ae')     #This parameter can be adjusted if you need it
    use_backup_system: int = 0     #This parameter can be adjusted if you need it
    use_backup_system_structural: int = 0     #This parameter can be adjusted if you need it
    running_stats_steps: int = 100       #This parameter needs to be adjusted
    str_prior_factor: float = 1.     #This parameter needs to be adjusted
    str_prior_temp: float = 0.1       #This parameter needs to be adjusted
    str_anneal: int = 0
    concat: str = choice('beam', 'sum', default='sum') # -
    beam_width: int = 1
    catch_outliers_old: bool = 1
    momentum_bn: float = 0.1      #This parameter needs to be adjusted
    ...
    #model relate
    hidden_size: int = 64      #This parameter needs to be adjusted
    module_type: str = 'conv'
    gating: str = choice('experts', 'locspec', default='locspec')
    num_modules: int = 2     #This parameter needs to be adjusted
    net_arch: int = choice('none', default='none') # -
    activation_structural: str = choice('sigmoid', 'relu', 'tanh', default='sigmoid')
    depth: int = 3         #This parameter needs to be adjusted
    use_bn: int = 1
    use_structural: int = 1
    ################
    
    #output module related (output leayer)
    multihead: str = choice('usual', 'gated_linear', 'gated_conv', 'none', default='gated_linear')
    normalize_oh: bool = 1 # -
    ...
    #########
    # ae          
    use_bn_decoder:int = 1
    momentum_bn_decoder: float = 0.1
    ...
    ##############################
    #Optimization
    wdecay: float = 0.001           #This parameter needs to be adjusted
    lr: float = 0.001 # learning rate           #This parameter needs to be adjusted
    ##############################
    #Logging

    pr_name: Optional[str]='dtf'
    wand_notes: str = ''
    log_avv_acc: int = 0
    ##############################
    ...
    stream_seed: int = 180
    n_tasks: int = 6
    task_sequence: str = choice('s_minus', 's_pl','s_plus', 's_mnist_svhn', 's_pnp_comp', 's_pnp_tr', 's_pnp', 's_in', 's_out', 's_long', 's_long30', 's_ood', default='s_in') #task sequence from ctrl
    batch_size: int = 64
    ...
    seed: int = 180
    debug: int = 0
    early_stop_complete: bool = 0
    warmup_bn_bf_training: int = 0 # -
    task_agnostic_test: int = 1

    keep_best_model: int = 1 # if 'True' keeps bestvalidation model
    num_epochs_added_after_projection:int = 10        #This parameter needs to be adjusted
    epochs_str_only_after_addition: int = 1
    epochs_structure_only_at_start: int = 0
    epochs: int =100       #This parameter needs to be adjusted
    shuffle_test: int = 0
    ##############################
    #EWC
    ewc_online: bool = 0.
...
def init_model(args:ArgsGenerator, gating='locspec', n_classes=10, i_size=28):
    ...
    if gating=='locspec':
        ...
        model_options.DTGN.no_projection_phase=args.no_projection_phase
        model_options.DTGN.init_stats=args.init_runingstats_on_addition
        model_options.DTGN.regime='normal'
        model_options.DTGN.lr=args.lr
        model_options.DTGN.wdecay=args.wdecay
        model_options.DTGN.depth=args.depth
        model_options.DTGN.lr_structural=args.lr_structural

        model_options.DTGN.net_arch=args.net_arch
        model_options.DTGN.n_modules=args.num_modules
        model_options.DTGN.temp=args.temp
        model_options.DTGN.str_prior_temp=args.str_prior_temp
        model_options.Module.n_heads_decoder=args.n_heads_decoder
        model_options.DTGN.fix_layers_below_on_addition=args.fix_layers_below_on_addition
        model_options.DTGN.module_type=args.module_type
        model_options.DTGN.str_prior_factor=args.str_prior_factor
        model_options.DTGN.concat=args.concat
        model_options.DTGN.beam_width=args.beam_width
        model_options.DTGN.catch_outliers_old=args.catch_outliers_old
        model_options.DTGN.module_init=args.module_init
        model_options.DTGN.multihead=multihead
        model_options.DTGN.deviation_threshold=args.deviation_threshold
        model_options.DTGN.mask_str_loss=args.mask_str_loss
        model_options.DTGN.projection_phase_length=args.projection_phase_length
        model_options.DTGN.optmize_structure_only_free_modules=args.optmize_structure_only_free_modules
        model_options.DTGN.automated_module_addition=1
        model_options.DTGN.active_unfreezing=args.active_unfreezing
        model_options.DTGN.unfreeze_structural=args.unfreeze_structural
        model_options.DTGN.treat_unfreezing_as_addition=args.treat_unfreezing_as_addition

        model = DTGN_net(model_options.DTGN,
...
def measurement(model, classes, test_loader, temp, str_prior_temp, task_id=None):
    model.eval()
    ...
    for i, (x,y) in enumerate(test_loader):
        ...
        forward_out, str_loss_SubDTGN_total= model(x, inner_loop=False, task_id=task_id, temp=temp, str_prior_temp=str_prior_temp, train_flag = False)
        str_loss_SubDTGN_total = str_loss_SubDTGN_total.detach().cpu().numpy()
        str_Loss_list_temp.append(str_loss_SubDTGN_total)
    ...
    return acc_test/len(test_loader), result, mask, str_Loss_list_temp
...
def train_on_task(model:nn.Module, args:ArgsGenerator, train_loader, valid_loader, test_loader, epochs=400, 
    ...
    str_Loss_list = []
    str_Loss_list_epoch = []
    len_loader_img: int = 0
    while e<epochs: 
        len_loader = len(train_loader)
        len_loader_img = len_loader
        loader = train_loader
        model.train()
        acc=0
        reg = 0
        ...
        for bi, batch in enumerate(loader):
            ...
            forward_out, str_loss_SubDTGN_total = model(x, inner_loop=False, task_id=task_id, temp=temp_e, str_prior_temp=str_temp_e, record_running_stats=True, detach_head=(e<_epochs_str_only_after_addition and e<epochs_structure_only_at_start), s=s)
        ...
        str_loss_mean = sum(str_Loss_list_temp)/len(str_Loss_list_temp)
        str_loss_mean = str_loss_mean
        str_Loss_list_epoch.append(str_loss_mean)
        ...
        if args.keep_best_model:
            if e>=_epochs_str_only_after_addition:
                ...
                if validate:
                    model.eval()
                    acc_valid, _, _, str_Loss_list_test = measurement(model, classes, valid_loader, temp=temp_e, str_prior_temp=str_temp_e, task_id=task_id)
        ...
        if e %5 == 0:
            ...
            model.eval()
            acc_test, result, _, str_Loss_list_test = measurement(model, classes, test_loader, temp=temp_e, str_prior_temp=str_temp_e, task_id=task_id if not args.task_agnostic_test else None)
            ...
            str_loss_mean = sum(str_Loss_list_test)/len(str_Loss_list_test)
            str_loss_mean = str_loss_mean
            str_Loss_list.append(str_loss_mean)
        e+=1
...
def measurement_with_bn(model, classes, test_loader, temp, str_temp, task_id=None, bn_warmup_steps=100):
    """ test mode with batchnomr warmup """
    model = bn_warmup(model, test_loader, task_id, bn_warmup_steps)  
    return measurement(model, classes, test_loader, temp, str_temp, task_id=task_id)
...
def get_accs_for_tasks(model, args:ArgsGenerator, loaders:List[DataLoader], accs_past: List[float]=None, task_agnostic_test: bool=False):
    ...
    for ti, test_loader in enumerate(loaders):    
        ...
        print(ti)
        acc, info, mask, str_Loss_list_test = measurement_with_bn(model, None, test_loader, model.min_temp, model.min_str_prior_temp, task_id=ti if not task_agnostic_test else None, bn_warmup_steps=steps_bn_warmup )
        acc = acc.cpu().item()
        accs.append(acc)
        masks.append(mask)
...
def train(args:ArgsGenerator, model, task_idx, train_loader_current, test_loader_current, valid_dataloader, fim_prev,er_buffer):
    ...
    test_acc = measurement(model, None, test_loader_current, model.min_temp, model.min_str_prior_temp, task_id=task_idx if not args.task_agnostic_test else None)[0].cpu().item()
    if best_valid_acc is None:
        valid_acc = measurement(model, None, valid_dataloader, model.min_temp, model.min_str_prior_temp, task_id=task_idx if not args.task_agnostic_test else None)[0].cpu().item()
    else:
...
def main(args:ArgsGenerator, task_gen:TaskGenerator):              
    ...
    if args.task_sequence_test is not None and 'ood' in args.task_sequence:
        ...
        for i, t in enumerate(task_gen_test):    
            ...
            test_acc, _, mask, str_Loss_list_test = measurement_with_bn(model, None, loader_test, model.min_temp, model.min_str_prior_temp, task_id=task_id if not args.task_agnostic_test else None, bn_warmup_steps=200)
            test_acc=test_acc.cpu().item()
            try:
                masks_test.append(mask.detach())
            except:
                masks_test.append(mask)
            valid_acc = measurement_with_bn(model, None, loader_valid, model.min_temp, model.min_str_prior_temp, task_id=task_id if not args.task_agnostic_test else None, bn_warmup_steps=100)[0].cpu().item()
...
if __name__== "__main__":                                     
    ...
    dfs=[]
    pr_name=f'DTGN' if args_generator.pr_name is None else args_generator.pr_name
